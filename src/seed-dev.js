...
const rushees = [
  {
    ...
    summary:
      'John fucking pulls!! Bid for sure. 5-5 chill to pull. Blah blah blah.',
    year: 'Fr',
  },
  {
    dorm: 'Serra',
...
const ratings = [{ rushee_id: 1, active_id: 1, value: 5 }];

const traits = [{ name: 'chill' }, { name: 'hype' }, { name: 'cs' }];
...
module.exports = async(function*(db) {
  ...
  yield Promise.all(
    rushee_trait_votes.map(rushee_trait_vote =>
      db.models.rushee_trait_vote.create(rushee_trait_vote),
    ),
  );
  yield Promise.all(
    rushee_comments.map(rushee_comment =>
      db.models.rushee_comment.create(rushee_comment),
    ),
  );
});
